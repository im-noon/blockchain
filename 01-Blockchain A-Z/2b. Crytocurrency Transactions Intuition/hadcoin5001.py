...
def is_valid():
    ...
    if is_valid:
        ...
        response = {'message': 'Houston,we have aproblem. The Blockchain is '
                               'invalid.'}
    return jsonify(response), 200
...
def add_transaction():
    ...
    response = {'message' :f'This transcaction will be add to block{index}'}
...
def connect_node():
    ...
    response = {
        'message': 'All the nodes are now connected. The Hadcoin Blockchain '
...
def replace_chain():
    is_chain_repalce = blockchain.replace_chain()
    if is_chain_repalce:
        response = {'message': 'The nodes had difference chains so the chain '
                               'was replaced by largest one.',
